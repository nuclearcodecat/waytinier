...
fn main() -> Result<(), Box<dyn Error>> {
	...
	let mut shm = SharedMemory::new_bound_initialized(&mut display, &mut registry, &mut wlmm, &mut wlim)?;
	let mut shm_pool =
	...
	while !ponged {
		...
		while let Some(ev) = wlmm.q.pop_front() {
			if ev.recv_id == xdg_wm_base.id && ev.opcode == 0 && let WireArgument::UnInt(serial) = ev.args[0] {
